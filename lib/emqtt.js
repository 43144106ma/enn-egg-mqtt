...
function createOneClient(config, app) {
  ...
  const mqttOptions = Object.assign(optionBase,config.options ? config.options : {}); 
  const mqttClient = mqtt.connect(config.host,mqttOptions);

  mqttClient.on('connect', () => {
    app.coreLogger.info('[enn-egg-mqtt] connected %s@%s:%s:%s/%s', config.host, config.username, config.password, config.clientId, config.options);
  });
  mqttClient.on('error', error => {
    app.coreLogger.error('[enn-egg-mqtt] error clientid:%s', config.clientId);
    app.coreLogger.error(error);
  });
  mqttClient.on('offline', () => {
    app.coreLogger.error('[enn-egg-mqtt] offline clientid:%s', config.clientId);
  });
  mqttClient.on('reconnect', () => {
    app.coreLogger.error('[enn-egg-mqtt] reconnect clientid:%s', config.clientId);
  });

  mqttClient.route = (topic, handler) => {
    mqttClient.subscribe(topic);
    app.coreLogger.info('[enn-egg-mqtt] subscribe clientid:%s topic:%s', config.clientId, topic);
    ...
    const topicAuth = new RegExp('^' + topic.replace('$queue/', '').replace(/^\$share\/([A-Za-z0-9]+)\//, '').replace(/([\[\]\?\(\)\\\\$\^\*\.|])/g, '\\$1')
      .replace(/\+/g, '[^/]+')
      .replace(/\/#$/, '(\/.*)?') + '$'); // emqtt兼容，共享订阅
    mqttClient.on('message', (top, message) => {
      ...
      msgMiddleware(app, request, msgMiddlewares, () => {
        ...
        handler.call(ctx)
          .catch(e => {
            e.message = '[enn-egg-mqtt] controller execute error: ' + e.message;
